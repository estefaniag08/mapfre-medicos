...
import { InfoPersonalPersona, ClienteInfo, PersonaCedula, RespuestaPersona, InformacionCompletaFinal, InformacionUrl, RespuestaUrl } from '../interfaces/PersonaCliente'
import { InformacionCotizacionMedico, RespuestaPolizaMedico, SarlafPreguntas } from './../interfaces/PolizaMedConsultas';
@Injectable({
  providedIn: 'root'
})
export class PersonaSolicitudesService {
  ...
  anadirPersona(infoPersona: InfoPersonalPersona) {
    ...
    this.http.post<RespuestaPersona>(url, infoPersona)
      .subscribe(respuesta => {
        this.idRegistroPersona = respuesta.id_persona;
      });
  }
  ...
  anadirInformacionCotizacion(infoCotizacion: InformacionCotizacionMedico) {
    const url = this.path + `/polizas-medicos/${this.idCotizacionMedico}/informacion-adicional`;
    this.http.put(url, {
  ...
  anadirInformacionCotizacionCorreo(infoCotizacion: InformacionCotizacionMedico) {
    const url = this.path + `/polizas-medicos/${this.idCotizacionMedico}/informacion-adicional`;
    this.http.put(url, {
      id_clase_rc: infoCotizacion.id_clase_rc,
      id_especialidad: infoCotizacion.id_especialidad,
      ...
      const url2 = this.path + `/polizas-medicos/${this.idCotizacionMedico}/correo`
  ...
  anadirInfoDocumentoPersona(infoDocumento: PersonaCedula) {
    const url = this.path + `/clientes/informacion-personal/${this.idRegistroPersona}`;
  ...
  anadirRespuestaPreguntas(infoPreguntas: SarlafPreguntas) {
    const url = this.path + `/polizas-medicos/${this.idCotizacionMedico}/sarlaf/preguntas`;
  ...
  obtenerUrlDocumentos() {
    const url = this.path + `/polizas-medicos/${this.idCotizacionMedico}/documentos`;
  ...
  adjuntarArchivo(cualDocumento: string, archivo: any, urlDoc: string) {
    ...
    if (this.idCotizacionMedico === undefined || this.idCotizacionMedico === null) {
      ...
      this.http.get<RespuestaUrl>(url).subscribe(rta => {
        ...
        const url = this.path + `/polizas-medicos/${this.idCotizacionMedico}/documentos/${cualDocumento}`;
