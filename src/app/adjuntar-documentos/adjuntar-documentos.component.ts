...
export class AdjuntarDocumentosComponent implements OnInit {
    ...
    if (file) {
      ...
      if (file.size < 5500000 && file.type == 'application/pdf') {
        ...
        reader.onload = () => {
          ...
          this.adjuntarDocumentosForm.get('copiaCedula').updateValueAndValidity();
          this.cd.markForCheck();
          this.labelCedula = 'Copia de tu cédula de ciudadanía';
          this.errorCedula = '';
        }
      } else if (file.size < 4500000 && (file.type == 'image/jpeg' || file.type == 'image/jpg' || file.type == 'image/png')) {
        this.fileNameCed = file.name;
        this.fileSelectedCed = 'ok';
        reader.onload = () => {
          this.adjuntarDocumentosForm.patchValue({
    ...
    if (file) {
      ...
      if (file.size < 5500000 && file.type == 'application/pdf') {
        ...
        reader.onload = () => {
          ...
          this.adjuntarDocumentosForm.get('copiaTarjetaProfesional').updateValueAndValidity();
          this.cd.markForCheck();
          //console.log(reader.result.toString());
        };
        ...
        file.size < 4500000 && (file.type == 'image/jpeg' || file.type == 'image/jpg' || file.type == 'image/png')
      ) {
        this.fileNameTarProf = file.name;
        this.fileSelectedTarProf = 'ok';
        reader.onload = () => {
    ...
    if (file) {
      ...
      if (file.size < 5500000 && file.type == 'application/pdf') {
        ...
        reader.onload = () => {
          ...
          this.adjuntarDocumentosForm.get('diplomaGrado').updateValueAndValidity();
          this.cd.markForCheck();
        };
  ...
  sendDocumentosForm() {
    if (this.adjuntarDocumentosForm.invalid) {
      ...
      /**
            this.personaService.adjuntarArchivo('documento_identidad',this.adjuntarDocumentosForm.get('copiaCedula').value,this.urlDoc).subscribe(
              rta => {
                console.log(rta);
                this.personaService.adjuntarArchivo('tarjeta_profesional',this.adjuntarDocumentosForm.get('copiaTarjetaProfesional').value,this.urlDoc).subscribe(
                  rta2=> {
                    console.log(rta2);
                    this.personaService.adjuntarArchivo('diploma_de_grado',this.adjuntarDocumentosForm.get('diplomaGrado').value ,this.urlDoc).subscribe(
                      rta3=> {
                        console.log(rta3);
                        this.personaService.adjuntarArchivo('acta_de_grado',this.adjuntarDocumentosForm.get('actaGrado').value, this.urlDoc).subscribe(
                          rta4=>{
                            console.log(rta4);
                          }
                        )
...
*/ 
      ...
      if (this.adjuntarDocumentosForm.controls['copiaTarjetaProfesional'].value !== null) {
        this.personaService.adjuntarArchivo('tarjeta_profesional', this.adjuntarDocumentosForm.get('copiaTarjetaProfesional').value, this.urlDoc).subscribe(
          rta2 => {
            //console.log(rta2);
          });
